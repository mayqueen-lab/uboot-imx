...
int board_mmc_init(bd_t *bis)
{
	...
	for (i = 0; i < CONFIG_SYS_FSL_USDHC_NUM; i++) {
		...
		case 1:
			...
			udelay(500);
			gpio_direction_output(USDHC2_PWR_GPIO, 1); /// nRST
...
#ifdef CONFIG_FEC_MXC
#define LAN8720_RST_GPIO	IMX_GPIO_NR(1, 8)
int board_eth_init(bd_t *bis)
{
	int ret;

	setup_iomux_fec(CONFIG_FEC_ENET_DEV);
	/// peixiuhui@163.com add 8720 gpio reset
	gpio_direction_output(LAN8720_RST_GPIO, 0);
	udelay(1000);
	gpio_direction_output(LAN8720_RST_GPIO, 1);
...
int checkboard(void)
{
	...
	puts("Board: PANZER Industrial PC\n");
...
static void spl_dram_init(void)
{
	...
	gpio_direction_input(DDR_TYPE_DET_GPIO5_1);
	gpio_direction_input(DDR_TYPE_DET_GPIO5_5);
	...
	**************************************/
	...
	if (gpio_get_value(DDR_TYPE_DET_GPIO5_1) && !gpio_get_value(DDR_TYPE_DET_GPIO5_5)) {
		debug("DRAM: 512MB\n");
	...
	} else if (!gpio_get_value(DDR_TYPE_DET_GPIO5_1) && !gpio_get_value(DDR_TYPE_DET_GPIO5_5)) {
		debug("DRAM: 256MB\n");
		...
		printf("DRAM: 128MB, this DRAM timing configuration doesn't exist!\n");
	} else
		printf("Fail to detect DRAM size!\n");
}
...
void board_init_f(ulong dummy)
{
	...
	/* Initialize GPMI and configure bch clock */
